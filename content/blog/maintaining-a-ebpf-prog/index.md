...
date: 2026-06-29
description: "Maintaining an eBPF program means surviving multiple kernel versions, architectures, Kconfig options, LLVM changes, and regressions. Here is what I learned while stabilizing tracexec’s eBPF backend."
...
The biggest maintenance burden is a kernel compatibility hell. It is very easy to write an eBPF program that only works with certain kernel versions/configurations. It is also known as the "it works on my machine" problem.

![It works on my machine (Picture Generated by ChatGPT)](./itworksonmymachine.png)

Regular userspace software also faces the same problem for similar but not the exact reasons and there is a lazy solution as illustrated by the following meme.

![Then let's ship your machine (Picture Generated by ChatGPT)](./shipyourmachine.png)
...
Now let's suppose that we are working on a syscall tracing eBPF program that needs to inspect user-space memory.
In some scenarios you might find it disappointing that [the kernel throws an `EFAULT` error at you when you try to read user-space memory](https://mozillazg.com/2024/03/ebpf-tracepoint-syscalls-sys-enter-execve-can-not-get-filename-argv-values-case-en.html).
...
on such a kernel config. [It turns out that the sleepable eBPF check is just using the error injection list
...
It might not be surprising that the syscall symbols look like `__x64_sys_execve`/`__arm64_sys_execve`/`__riscv_sys_execve`.
...
- Second, tracepoints do not support sleepable eBPF programs (at least not available on stable kernels for now).
...
- Third, tracepoints only handle the native syscall interface. If you want to handle any alternative syscall interfaces, you will need `fprobe`/`kprobe` anyway. This is explained in detail in the next subsection.
...
Before Eduard took a detailed look, I had already find a workaround.
...
[A lot of kernel verifier changes are tested upon Cilium eBPF programs](https://pchaigno.github.io/ebpf/2025/09/23/test-verifier-changes-on-cilium-bpf-programs.html). But other eBPF programs do not enjoy such support.
...
So the natural solution is implement both program types and dynamically choose which one to load
...
Unfortunately, I know little about handling LLVM major version upgrades for an LLVM pass plugin.
...
- Latest RC release if one exists.
