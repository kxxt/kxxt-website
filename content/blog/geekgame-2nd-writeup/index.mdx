---
title: Geekgame 2nd WriteUp
date: 2022-11-27
tags: [geekgame, ctf, writeup, security]
description: Geekgame 2nd 的题解
published: true
---
...
第 4 题：视频 bilibili.com/video/BV1EV411s7vu 也可以通过 bilibili.com/video/av**\_** 访问。下划线内应填什么数字？
...
```js
javascript: document["getElementsByClassName"]`flag`[0]["innerText"][i][
  "endsWith"
]`ch`
...
```js
javascript: document["getElementsByTagName"]`title`[0]["append"]`${
  document["getElementsByClassName"]`flag`[0]["innerText"]
}` //
```
...
这相当于调用了 `title.append([""], "FLAG 内容")` 。非常的奇妙，竟然能直接 `append` 多个变量进去，我 <sup>TM</sup> 感觉我之前白费了几个小时。

第一个 flag 不会拿。我写了段能把标题改成那段文本的 JS , 感觉没有办法把它伪装成 QUERY：

```js
javascript: document["getElementsByTagName"]`title`[0]["childNodes"][0][
  "remove"
]``,
  document["getElementsByTagName"]`title`[0][
    "append"
  ]`Unsafe${"GIVE-ME-FLAG-1 #"}${String["fromCharCode"]`61`}${"_"}${String[
    "fromCharCode"
  ]`61`}${" @!+!@ ^~^ "}${String["fromCharCode"]`37`}${"[*"}${String[
    "fromCharCode"
  ]`46`}${"*]"}${String["fromCharCode"]`37`}`,
  document["getElementsByTagName"]`title`[0]["childNodes"][0]["remove"]`` //
```
...
把表单的 _`type`_ 字段改为 _`PKU_GeekGame`_ ，做个 POST 请求，即可拿到 flag1.
...
去看看 _`/admin/source_bak/`_ 这个接口下载下来的源代码。

不难发现 _`/admin/{index}/`_ 这个路由会给 _`http://localhost:8079/{index}`_ 发请求，请求的 body 是 _`queryBean`_ 的 type。响应会被放到 _`queryBean`_ 的 value 里。

那么我们怎么向 _`http://localhost:8080`_ 发送请求呢？

经过尝试，得到一个方案（_`%3A`_ 是冒号的 URL 转义）：
...
我们发现 8080 端口上运行着 _`/bonus`_ 和 _`/source_bak`_ 两个路由，可是我们怎么访问它们呢。
...
再把 _`%`_ 给 转义一遍，得到 _`%25`_, 跟原来的 _`2F`_ 接起来，得到 _`%252F`_.

这样的话， URL 在过 Nginx 的时候， _`%252f`_ 变成 _`%2f`_, 再被 jvav 解码一遍就变成了 _`/`_.

那我们就能给 _`/bonus`_ 发请求了
...
然后我们去看一看源码备份 _`/source_bak`_. 把源码下载到 _`source.java`_
...
不难发现 _`/bonus`_ 路由会在 _`type==CommonsText`_ 时对 _`queryBean`_ 里的 _`value`_ 执行一个字符串变量插值（在去除危险词汇之后）。
...
因为去除危险词汇的函数只遍历了一遍，所以我们可以写 _`jajsva`_，它被替换之后就是 _`java`_ 这个危险词汇。

我 Google 搜了一下 _`StringSubstitutor`_， 发现它能直接把文件插进字符串里，然而 _`file`_ 并不在危险词汇里。。。不知道这算不算非预期做法。。去除危险词汇的逻辑丝毫没被我碰到。
...
或许我们可以把 _`forbid`_ 的限制给绕过去？

试了试 _`#[allow(unsafe_code)]`_ 结果不行。。Google 搜了搜， 找到了 _`#[allow_internal_unsafe]`_ ， 结果也不行（在低版本的 rust 里可以，不过[这个问题后来被修了](https://github.com/rust-lang/rust/issues/56768)）。
...
后来想到了 ELF 的 _`.init`_ 段和 _`.fini`_ 段。_`.init`_ 段的代码会在程序启动前执行。说不定我能把一个 Rust 函数链接到 _`.init`_ 段,这正好也绕过了沙盒对系统调用的限制。
...
不过使用题目提供的容器里的编译器编译并不会出现编译错误，应该是高版本的 rust 编译器在 _`#![forbid(unsafe_code)]`_ 控制范围的代码里禁止了这种行为。
...
诶？？？你竟然可以在 Safe Rust（高版本的 Rust 已经在 _`#![forbid(unsafe_code)]`_ 的情况下禁止这种行为） 里往 ELF 里链接一段机器码？？？
...
我用 pwntools 生成了一段读取并输出 _`/flag2`_ 的机器码（因为我太菜了，不会写 Shellcode）。
...
最后几个字符解不出来，气死了。。。(字有点丑。。。请无视)

![image-20221123205050608](assets/image-20221123205050608.png)

我还用我写的 [`aspeak`](https://github.com/kxxt/aspeak) 合成语音片段，中间插上空白，尝试了一下 我用 108 天录了个音 这道题。然而并没有过。
